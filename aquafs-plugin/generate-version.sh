...
updateVersionFile () {
  if [ "${#VERSION}" -gt 63 ]; then
    echo "The 'git describe --abbrev=7 --dirty' generated version is longer than 63 chars, thus not fitting in the AquaFS superblock. Aborting!" >&2
    return 1
  fi
  printf '#pragma once\n#define AQUAFS_VERSION "%s"\n' \
...
if [ -f "$REPO_ROOT/fs/version.h" ]; then
  PERSISTED_VERSION=$(cat $REPO_ROOT/fs/version.h | grep ' AQUAFS_VERSION ' | grep -o '".*"' | sed 's/"//g')
